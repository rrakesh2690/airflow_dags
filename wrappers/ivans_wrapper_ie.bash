...
########################################################################################################
#    Bash script to load segments from IE.dat file on mount point to Bigquery IVANS tables             #
#Steps:																							       #	
#1.Check for current date file for IE in mount point. 											       #	
...
MOUNT_POINT ='/app/GC_SDK/sys_ig/policy_none_as400'
...
#Check date is equivalent to current date
	...
	if [[ $raw_filename = *$(date +"%Y%m%d")* ]]; then
		...
		echo $current_date ": .DAT file of IE ="$raw_filename                     >> $logfile
		...
		# Processing of IE .DAT file and data load to Bigquery
		filename=$(echo $raw_filename | rev | cut -d"/" -f1 | rev)  >> $logfile
		
        python ${scriptdir}/load_ie_segments_to_bq_dataflow.py --config "config.properties" --productconfig "ivans.properties" --env ${ENV} --separator "|" --stripheader "0" --stripdelim "0" --addaudit "1" --writeDeposition "WRITE_APPEND" --system "IE" --input "${BUCKET_NAME}${filename}"  >> $logfile
