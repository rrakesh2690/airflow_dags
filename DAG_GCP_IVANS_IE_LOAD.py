...
with DAG('DAG_GCP_IVANS_IE_LOAD', schedule_interval='@daily', catchup=False, default_args=default_args) as dag:
#     t1 = BashOperator(
...
#     )
    ...
    t2 = BashOperator(
        task_id='T2_GCP_LOAD',
        bash_command='python /home/airflow/gcs/data/GCPDWH/ivans/load_ie_segments_to_bq_dataflow.py --config "config.properties" --productconfig "ivans.properties" --env "dev" --separator "|" --stripheader "0" --stripdelim "0" --addaudit "1" --writeDeposition "WRITE_APPEND" --system "IE" --input ' + filename)
